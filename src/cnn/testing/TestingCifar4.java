...
//                System.out.println(data[i][5]);
            ...
            Options[] defs = new Options[8];
            defs[0] = new Options();
            defs[0].put("type", "input");
            defs[0].put("out_sx", 32);
            defs[0].put("out_sy", 32);
            defs[0].put("out_depth", 3);

            defs[1] = new Options();
            defs[1].put("type", "conv");
            defs[1].put("sx", 5);
            defs[1].put("filters", 16);
            defs[1].put("stride", 1);
            defs[1].put("pad", 2);
            defs[1].put("activation", "relu");

            defs[2] = new Options();
            defs[2].put("type", "pool");
            defs[2].put("sx", 2);
            defs[2].put("stride", 2);

            defs[3] = new Options();
            defs[3].put("type", "conv");
            defs[3].put("sx", 5);
            defs[3].put("filters", 20);
            defs[3].put("stride", 1);
            defs[3].put("pad", 2);
            defs[3].put("activation", "relu");

            defs[4] = new Options();
            defs[4].put("type", "pool");
            defs[4].put("sx", 2);
            defs[4].put("stride", 2);

            defs[5] = new Options();
            defs[5].put("type", "conv");
            defs[5].put("sx", 5);
            defs[5].put("filters", 20);
            defs[5].put("stride", 1);
            defs[5].put("pad", 2);
            defs[5].put("activation", "relu");

            defs[6] = new Options();
            defs[6].put("type", "pool");
            defs[6].put("sx", 2);
            defs[6].put("stride", 2);

            defs[7] = new Options();
            defs[7].put("type", "softmax");
            defs[7].put("num_neurons", 10);

            Net net = new Net();
            net.makeLayers(defs);

            Options opt = new Options();
            opt.put("method", "adadelta");
            opt.put("batch_size", 1);
            opt.put("l2_decay", 0.0001);
            Trainers trainer = new Trainers(net, opt);

            int N = 50;
            int max_iter = 200;
            Vol netx = new Vol(32, 32, 3,0);
            double avloss = 0.0;
